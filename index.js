...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`𝙲𝙰𝚁𝚅𝙸𝙽 𝚂𝙸𝚁 😎`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = xeon.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*Carvin*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
...
│
│  ❑ *ɢɪᴛʜᴜʙ ʟɪɴᴋ* :
│  https://github.com/Carvinsir/Carvin-SIR
│  
│  ❑ *ʏᴏᴜᴛᴜʙᴇ ʟɪɴᴋ* :
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await xeon.downloadMediaMessage(encmedia)
						xeon.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 599963135602})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await xeon.downloadMediaMessage(encmedia)
						xeon.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 599963135602, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
                    ...
                    xeon.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 599963135602, caption: `${konti}`})
...
if (Number(args[0]) >= 50) return reply('Most!')
	...
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  xeon.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 599963135602, ptt:true})
	  }
